...
class BaseAPIView(APIView):
    ...
    def send_success_response(self, msg_code, message, data=None, pagination=None, errors_details=None):
        """Compose success response."""
        return self.send_response(status.HTTP_200_OK, msg_code, message, data, pagination, errors_details)

    def send_created_response(self, msg_code, message, data=None, pagination=None, errors_details=None):
        """Compose response for new object creation."""
        return self.send_response(status.HTTP_201_CREATED, msg_code, message, data, pagination, errors_details)

    def send_bad_request_response(self, msg_code, message, errors_details=None):
        """Compose response for bad request."""
        return self.send_response(status.HTTP_400_BAD_REQUEST, msg_code, message, errors_details=errors_details)

    def send_not_found_response(self, msg_code, message="Not found.", errors_details=None):
        """Compose response for not found request."""
        return self.send_response(status.HTTP_404_NOT_FOUND, msg_code, message, errors_details=errors_details)

    def send_internal_server_error_response(self, msg_code, message="Internal server error.", errors_details=None):
        """Compose response for internal server error response."""
        return self.send_response(status.HTTP_500_INTERNAL_SERVER_ERROR, msg_code, message, errors_details=errors_details)
